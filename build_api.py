...
def run_job(current_job: Job, work_dir: str) -> str:
    """
...
def start_build_system(config_path: str, artefacts_dir: str) -> str:
    """
    :param config_path: путь к файлу конфигурации
    :param artefacts_dir: папка артефактов
    :return: результат в виде строки в формате JSON
    """
    ...
    ready_jobs_set = set()  # Множество готовых задач
    ...
    while True:
        ...
        if len(jobs.run_queue) == 1:
            ...
            for job_name in jobs.run_queue:
                ...
                if not set(jobs[job_name].depends_on) - ready_jobs_set:
                    parallel_run.append(job_name)
                if len(parallel_run) == 3:
                    break

        # Создаём временные директории для каждой из задач
        tmp_wrk_dirs |= {job_name: TemporaryDirectory() for job_name in parallel_run}  # "|=" работает для (python 3.9+)
        """
        ...
        with ThreadPool(len(parallel_run)) as p:
            res_runs = list(
                p.starmap(run_job, iterable=[(jobs[name], tmp_wrk_dirs[name].name) for name in parallel_run]))
        ...
        ready_jobs_set |= set(parallel_run)
